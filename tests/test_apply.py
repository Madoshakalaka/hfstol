...
@pytest.mark.parametrize('surface_forms,result', [
    (
...
@pytest.mark.parametrize('surface_form,result', [
    (
            ...
            (('k', 'i', 'n', 'i', 'p', 'â', 'n', 'a', 'w',), ('k', 'i', 'n', 'i', 'p', 'â', 'n', 'â', 'n', 'a', 'w',))
    ),
])
def test_single_generation_multiple_deep_form_split(cree_generator, surface_form, result):
    assert cree_generator.feed(surface_form, concat=False) == result
...
@pytest.mark.parametrize('surface_forms,result', [
    (
            ...
            {'niska+N+A+Pl': {('niskak',)}, 'nipâw+V+AI+Ind+Prs+12Pl': {('kinipânaw',), ('kinipânânaw',)},
             'abcdefg': set(), '': set()}
    )
])
...
@pytest.mark.parametrize('surface_forms,result', [
    (
            ["niska+N+A+Pl", 'nipâw+V+AI+Ind+Prs+12Pl', 'abcdefg', ''],
            {'niska+N+A+Pl': {('n', 'i', 's', 'k', 'a', 'k')},
             'nipâw+V+AI+Ind+Prs+12Pl': {('k', 'i', 'n', 'i', 'p', 'â', 'n', 'a', 'w'),
                                         ('k', 'i', 'n', 'i', 'p', 'â', 'n', 'â', 'n', 'a', 'w')}, 'abcdefg': set(),
             '': set()}
...
@pytest.mark.parametrize('surface_forms,result', [
    (
            ["niska+N+A+Pl", 'nipâw+V+AI+Ind+Prs+12Pl', 'abcdefg', ''],
            {'niska+N+A+Pl': {'niskak'}, 'nipâw+V+AI+Ind+Prs+12Pl': {'kinipânaw', 'kinipânânaw'}, 'abcdefg': set(),
             '': set()}
    )
])
